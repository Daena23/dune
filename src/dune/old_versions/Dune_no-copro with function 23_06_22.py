...
for step_of_each_monster in range(len(remove_init_position)):
    if remove_init_position[step_of_each_monster] in monster_available_cells:
        monster_available_cells.remove(remove_init_position[step_of_each_monster])
...
for step_of_each_monster in range(n_monster):
    ...
    monster_list_init.append([y_monster, x_monster])  # создаем лист с координатами монстров
...
for step in range(10):
    ...
    for step_of_each_monster in range(n_monster):
        y_monster, x_monster = monster_list_curr[step_of_each_monster]
        # print('y_monster, x_monster=', y_monster, x_monster)
        monster_new_coord = direction_choice(y_monster, x_monster, penetratable_cell_coordinates)

        y1_monster = monster_new_coord[0]
        x1_monster = monster_new_coord[1]
        lines[y1_monster][x1_monster] = 3
        lines[y_monster][x_monster] = 0
        monster_list_curr[step_of_each_monster] = y1_monster, x1_monster

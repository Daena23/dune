...
class MonsterCat(Monster):
    ...
    def move(self, penetrable_cell_coord, step):
        direction_variants = direction_variants_identification(self.y_monster, self.x_monster, penetrable_cell_coord)
        if step % 2 == 0:
...
class MonsterDog(Monster):
    ...
    def move(self, penetrable_cell_coord, step):
        ...
        if self.direction == 'undefined' or self.direction == 'no_way':
            ...
            monster_new_coord = action_straight(self.x_monster, self.y_monster, self.direction, direction_variants,
...
def direction_variants_identification(y_monster, x_monster, penetrable_cell_coord):
...
def action_straight(x_monster, y_monster, monster_direction, direction_variants, penetrable_cell_coord):
    ...
    for direction_id in range(len(COORDINATE_VARIANTS)):
        ...
        if monster_direction == dir_name and [y_sum, x_sum] in penetrable_cell_coord:
        ...
        elif monster_direction == dir_name and [y_sum, x_sum] not in penetrable_cell_coord:
...
class Player:
    ...
    def player_move(self, penetrable_cell_coord):
        if self.player_alive:
            ...
            for i in range(len(PLAYER_COORDINATE_VARIANTS)):
                ...
                if event == PLAYER_COORDINATE_VARIANTS[i][3] and [y_player_new, x_player_new] in penetrable_cell_coord:
    ...
    def bomb_positioning(self, bomb):
...
class Lines:
    ...
    def monster_coord_ident(self):
        ...
        for counter in range(n_monster):
            ...
            self.monster_list_init.append([y_monster, x_monster, 'undefined'])
    ...
    def playfield_update(self, bomb, player, my_monsters):
        ...
        if bomb.bomb_timer == bomb.explosion_time:
            ...
            bomb.bomb_timer = 0
            bomb.explosion_area = []
            bomb.bomb_exist = False
            bomb.breakable_walls_for_removal_coord = []
...
class Bomb:
    ...
    # my_bombs = bombs.append()



    #
    # def put_bomb(self, lines, bomb_description, step, refractory_period):
    #     y_bomb = bomb_description[0]
    #     x_bomb = bomb_description[1]
    #     lines.field[y_bomb][x_bomb] = self.id
    #     timer = step + 10
    #     bomb_timer = 0
    #     bomb_timer_end = 10
    #     bomb_description = [y_bomb, x_bomb, bomb_timer]
    #     bomb_timer = bomb_timer + 1
    #     print(bomb_timer, refractory_period)
    #     if bomb_timer > bomb_timer_end:
    #         refractory_period = False
    #     return bomb_description

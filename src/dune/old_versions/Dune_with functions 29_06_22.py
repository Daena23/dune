...
for step in range(10):
    for counter in range(n_monster):
        ...
        monster_direction = monster_list_curr[counter][2]
        # if m1.type == 'cat':
        #             # elif m1.type == 'dog':
        if monster_direction == 'undefined' or monster_direction == 'no_way':
            direction_variants = direction_variants_identification(y_monster, x_monster, penetratable_cell_coordinates)
            monster_new_coord = direction_choice(direction_variants, x_monster, y_monster)
        elif monster_direction == 'up' or monster_direction == 'down' or monster_direction == 'left' or monster_direction == 'right':
            monster_new_coord = action_straight(x_monster, y_monster, monster_direction, penetratable_cell_coordinates)
        y1_monster = monster_new_coord[0]
        x1_monster = monster_new_coord[1]
        monster_direction = monster_new_coord[2]
        lines[y_monster][x_monster] = 0
        lines[y1_monster][x1_monster] = 3
        monster_list_curr[counter] = y1_monster, x1_monster, monster_direction

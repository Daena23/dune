...
class Explosion(AnyObject):
    def __init__(self, game: Game, bomb_row: int, bomb_column: int):
        super().__init__()
        self.symbol = SYMBOLS['Explosion']
        self.explosion_power = LEVEL_CONSTANTS['explosion_power']
        self.exists = True
        # Coordinates
        self.row = bomb_row
    ...
    def find_area(self, game: Game) -> List[List[int]]:
        ...
        for row_dir, column_dir in COORD_VARS[0:4]:
            for explosion_power in range(1, LEVEL_CONSTANTS['explosion_power']):
                row_beam, column_beam = self.row + row_dir * explosion_power, self.column + column_dir * explosion_power
...
def hits_portal(field: Field, game: Game, row_beam: int, column_beam: int) -> None:
    if [game.portal.row, game.portal.column] == [row_beam, column_beam] and\
        ...
        game.portal.is_under_explosion = True
...
def destroy(obj: AnyObject) -> None:
    obj.exists = False

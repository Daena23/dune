...
class Player(LivingObject):
    def __init__(self):
        super().__init__(*PLAYER_INIT_COORD[0])
        # coord
        self.init_row = 1
        self.init_column = 1
        self.row = 1
    ...
    def make_step(self, game: Game, event: str) -> None:
    ...
    def put_bomb(self, game: Game) -> None:
        passable_cell = [game.player.row, game.player.column] in game.penetrable_cell_coord
        if passable_cell and [self.previous_row, self.previous_column] != [game.portal.row, game.portal.column]:
            create_bomb(game)
